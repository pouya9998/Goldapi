...
local json = (loadfile "/home/username/Goldcli/data/JSON.lua")()
local token = '' --token
    ...
    return false
              ...
              local keyboard = {}
							keyboard.inline_keyboard = {
								{
                 {text = 'تنظیمات گروه', callback_data = 'groupsettings:'..chat}
				  },{
				 {text = 'اطلاعات گروه', callback_data = 'groupinfo:'..chat},{text = 'پشتیبانی', callback_data = 'supportbot:'..chat},{text = 'راهنما', callback_data = 'helpbot:'..chat}
				}
							}
          ...
          if msg.callback_query then
            local q = msg.callback_query
             ...
             if q.data:match('_') and not (q.data:match('next_page') or q.data:match('left_page')) then
                Canswer(q.id,"برای مشاهده راهنمای بیشتر این بخش عبارت\n/help\nرا ارسال کنید\nتیم پشتیبانی:[@Sourcegroupbot]\nکانال پشتیبانی:[@StourTeam]\n فروش:[@Releas_Dev]",true)
          ...
          spam_status = 'اخراج سازي(کاربر)'
					redis:hset("flooding:settings:"..chat, "flood",'kick')
								...
								if tonumber(MSG_MAX) == 20 then
									Canswer(q.id,'حداکثر عدد انتخابي براي اين قابليت [20] مي باشد',true)
									else
							  ...
							  result = MSG_MAX
								...
								if tonumber(MSG_MAX) == 2 then
									Canswer(q.id,'حداقل عدد انتخابي مجاز  براي اين قابليت [2] مي باشد',true)
									else
								MSG_MAX = tonumber(MSG_MAX) - 1
								redis:hset("flooding:settings:"..chat,"floodmax",MSG_MAX)
								q.data = 'next_page:'..chat
								...
								Canswer(q.id,'حداکثر عدد انتخابي براي اين قابليت [10] مي باشد',true)
									else 
 ...
 if redis:get(hash) then
    ...
    return false
		...
		local keyboard = {}
							keyboard.inline_keyboard = {
								{
				 {text = 'گزارش مشکل', callback_data = 'reportproblem:'..chat}
                },{
		...
		local keyboard = {}
              ...
              edit(q.inline_message_id,'`با عرض پوزش،در حال حاضر سیستم انتخابی غیرفعال می باشد`',keyboard)
            end
							------------------------------------------------------------------------
							------------------------------------------------------------------------
          ...
          return '❌'
              ...
              local keyboard = {}
            	keyboard.inline_keyboard = {
                 ...
                 {text=getsettings('hashtag'),callback_data=chat..':lock hashtag'}, {text = 'فیلتر هشتگ', callback_data = chat..'_hashtag'}
                },{
